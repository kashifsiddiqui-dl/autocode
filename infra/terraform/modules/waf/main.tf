################################################################################
# WAF Module – Auto Code Web Application Firewall
################################################################################
...
resource "aws_wafv2_web_acl" "main" {
  ...
  description = "WAF Web ACL for Auto Code ${var.environment}"
  scope       = "REGIONAL"
  ...
  rule {
    ...
    visibility_config {
      cloudwatch_metrics_enabled = true
      metric_name                = "${var.project_name}-${var.environment}-missing-ua"
      sampled_requests_enabled   = true
    }
  }
